...
export default class Auth extends Component {
	...
	//Chama tela de Cadastro ou LOGIN
		...
		return (
			<ImageBackground source={backgroundImage} style={styles.background}>
				...
				<View style={styles.formContainer}>
					<Text style={styles.subtitle}>{this.state.stageNew ? 'Criar sua Conta' : "Informe Login e Senha"}</Text>
				...
				<TouchableOpacity style={{ padding: 10 }} onPress={() => this.setState({ stageNew: !this.state.stageNew })}>
					<Text style={styles.buttonText}>{this.state.stageNew ? 'Já possue conta?' : 'Ainda não possue conta?'}</Text>
				</TouchableOpacity>
			</ImageBackground>
